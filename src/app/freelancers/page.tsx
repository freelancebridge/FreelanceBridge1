    ...
    try {
        ...
        const dbUsers = await (async () => {
            try {
                ...
                console.error('DATABASE ERROR in FreelancersList:', error);
                return []; 
            }
        })();
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white p-8 rounded-3xl shadow-xl max-w-2xl w-full border border-red-100">
                    <h1 className="text-3xl font-bold text-red-600 mb-4">Something went wrong (500)</h1>
                    <p className="text-gray-600 mb-6">We encountered a runtime error on the /freelancers page. This has been logged for our engineers.</p>
                    <div className="bg-red-50 p-6 rounded-2xl overflow-auto mb-6">
                        <p className="font-bold text-red-800 mb-2">Error Details:</p>
                        <code className="text-red-700 text-sm whitespace-pre-wrap">{error.message || String(error)}</code>
                    </div>
                </div>
            </div>
        );
