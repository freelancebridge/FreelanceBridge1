...
export default async function FreelancersList({ searchParams }: { searchParams: any }) {
    try {
        // Handle both Next.js 14 (object) and Next.js 15 (Promise) searchParams
        ...
        const dbUsers = await (async () => {
            try {
                ...
                console.error('Database connection error in FreelancersList Prisma call:', error);
        ...
        console.error('Fatal error in FreelancersList:', e);
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
                <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg w-full border border-red-100">
                    <h2 className="text-2xl font-bold text-red-600 mb-4">Page Error (500)</h2>
                    <p className="text-gray-600 mb-6">We encountered an issue loading the wizards list. This has been logged.</p>
                    <div className="bg-red-50 p-4 rounded-lg overflow-auto max-h-40 mb-6">
                        <code className="text-red-800 text-sm">{e.message}</code>
                    </div>
                    <Link href="/" className="inline-block px-6 py-3 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 transition-colors w-full text-center">
                        Back to Homepage
                    </Link>
                </div>
            </div>
